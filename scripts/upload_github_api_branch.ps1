...
if ($args.Length -lt 3) {
  throw "Usage: upload_github_api_branch.ps1 <branch> <commit-message> <file> [file...]"
}

$Branch = $args[0]
$Message = $args[1]
$Files = $args[2..($args.Length - 1)]
...
$Ref = Invoke-GhJson "GET" "repos/$Repo/git/ref/heads/$Branch"
...
Invoke-GhJson "PATCH" "repos/$Repo/git/refs/heads/$Branch" @{
  sha = $NewCommit.sha
  force = $false
} | Out-Null
